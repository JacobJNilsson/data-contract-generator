// Package jsoncontract analyzes JSON and NDJSON files and produces
// source contracts describing their structure, encoding, schema, and
// data quality. Supports both JSON arrays and newline-delimited JSON.
package jsoncontract

// SourceContract is the complete analysis result for a JSON/NDJSON file.
type SourceContract struct {
	SourceFormat string     `json:"source_format"` // "json" or "ndjson"
...
// Field describes a single field discovered across the JSON objects.
type Field struct {
...
// DataType represents the inferred type of a field.
...
type Options struct {
	...
	// field for frequency counting. Zero means use the default (10000).
	MaxTracked int
	...
	// MaxRows is the maximum number of objects to read. Zero means
	// read all objects (for JSON arrays) or the entire file (for NDJSON).
	MaxRows int
}
